   ...
   public class MemoryCacheWithSizeLimitTests
    {
        ...
        {
            using (MemoryCacheWithSizeLimit<int> cache = new MemoryCacheWithSizeLimit<int>(1))
            {
                int value;
        ...
        {
            using (MemoryCacheWithSizeLimit<string> cache = new MemoryCacheWithSizeLimit<string>(5))
            {
        ...
        {
            using (MemoryCacheWithSizeLimit<string> cache = new MemoryCacheWithSizeLimit<string>(4))
            {
        ...
        public void Smokey()
        {
            using (MemoryCacheWithSizeLimit<string> cache = new MemoryCacheWithSizeLimit<string>(5))
            {
                ...
                Assert.That(cache.GetOrAdd("1", key => "1ValueUpdated"), Is.EqualTo("1Value").Or.EqualTo("1ValueUpdated"));
                    //because of the delayed trim of the cache
